                          ...
                          int writable)
{
    ...
    if (method == mlt_deinterlacer_weave || method == mlt_deinterlacer_greedy
    ...
    if (srcimg.data) // Maybe already received during progressive check
    {
        ...
        error = mlt_frame_get_image(frame,
                                    (uint8_t **) &srcimg.data,
                                    &srcimg.format,
                                    &srcimg.width,
                                    &srcimg.height,
    ...
    if (pdata->prev_next_required) {
        ...
        if (prevframe) {
            ...
            error = mlt_frame_get_image(prevframe,
                                        (uint8_t **) &previmg.data,
                                        &previmg.format,
                                        &previmg.width,
                                        &previmg.height,
                                        0);
        ...
        if (nextframe) {
            ...
            error = mlt_frame_get_image(nextframe,
                                        (uint8_t **) &nextimg.data,
                                        &nextimg.format,
                                        &nextimg.width,
                                        &nextimg.height,
                                        0);
