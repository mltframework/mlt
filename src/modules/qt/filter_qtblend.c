/*
 * filter_qtblend.c -- qtblend filter
	...
	*format = mlt_image_rgb24a;
	...
	if ( error == 0 )
	{
		...
		if ( producer != NULL && transition != NULL )
		{
			...
			mlt_properties_set_data( frame_properties, "qtblend_frame", a_frame, 0, (mlt_destructor)mlt_frame_close, NULL );
...
mlt_filter filter_qtblend_init( mlt_profile profile, mlt_service_type type, const char *id, char *arg )
{
	...
	if ( filter != NULL )
	{
		...
		mlt_properties_set( MLT_FILTER_PROPERTIES( filter ), "background", "colour:0" );
	}
	return filter;
}
