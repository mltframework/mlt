...
int refresh_qimage(producer_qimage self, mlt_frame frame, int enable_caching)
{
    ...
    // Image index
    int image_idx = (int) floor((double) position / mlt_properties_get_int(producer_props, "ttl"))
                    % self->count;
    ...
    if (!self->qimage || mlt_properties_get_int(producer_props, "_disable_exif") != disable_exif) {
        ...
        if (!qimage->isNull()) {
            ...
            mlt_properties_set_int(producer_props,
                                   "format",
                                   qimage->hasAlphaChannel() ? mlt_image_rgba : mlt_image_rgb);
...
void refresh_image(producer_qimage self,
                   mlt_frame frame,
                   ...
                   int enable_caching)
{
    ...
    if (!enable_caching || image_idx != self->image_idx || width != self->current_width
    ...
    if (self->qimage
        ...
        if (enable_caching) {
            ...
            mlt_service_cache_put(MLT_PRODUCER_SERVICE(producer),
                                  ...
                                  image_size,
                                  mlt_pool_release);
            self->image_cache = mlt_service_cache_get(MLT_PRODUCER_SERVICE(producer),
                                                      "qimage.image");
...
extern void make_tempfile(producer_qimage self, const char *xml)
{
    ...
    if (tempFile.open()) {
        ...
        mlt_properties_set_data(MLT_PRODUCER_PROPERTIES(&self->parent),
                                "__temporary_file__",
                                fullname.data(),
                                0,
...
int load_folder(producer_qimage self, const char *filename)
{
    ...
    if (strstr(filename, "/.all.") != NULL) {
        ...
        for (auto &f : files) {
            ...
            mlt_properties_set_string(filename_property,
                                      QString::number(key).toLatin1(),
                                      dir.absoluteFilePath(f).toUtf8().constData());
        }
        result = 1;
    }
    return result;
