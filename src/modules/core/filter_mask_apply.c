...
static mlt_frame process(mlt_filter filter, mlt_frame frame)
{
    ...
    if (!transition || !mlt_properties_get(MLT_FILTER_PROPERTIES(transition), "mlt_service")
    ...
    if (transition) {
        ...
        if ((type & 1) && !mlt_frame_is_test_card(frame) && !(hide & 1)) {
            mlt_frame_push_service_int(frame,
                                       mlt_image_format_id(
                                           mlt_properties_get(properties, "mlt_image_format")));
...
mlt_filter filter_mask_apply_init(mlt_profile profile,
