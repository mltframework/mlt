                     ...
                     int writable)
{
    ...
    if (c_frame) {
        ...
        *format = mlt_image_movit;
        ...
        mlt_properties_set_double(properties,
                                  "_movit.parms.float.strength_first",
                                  reverse ? mix : inverse);
        mlt_properties_set_double(properties,
                                  "_movit.parms.float.strength_second",
                                  reverse ? inverse : mix);
    ...
    if (producer) {
        ...
        if (mlt_service_get_frame(MLT_PRODUCER_SERVICE(producer), &wipe, 0) == 0) {
            ...
            snprintf(name,
                     sizeof(name),
                     "movit.luma %s",
                     mlt_properties_get(properties, "_unique_id"));
...
extern "C" mlt_transition transition_movit_luma_init(mlt_profile profile,
