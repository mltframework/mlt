...
static void show_frame()
{
    ...
    glTexCoord2f(0.0f, 1.0f);
    glVertex2f(0.0f, new_frame.height);
    glTexCoord2f(1.0f, 1.0f);
    glVertex2f(new_frame.width, new_frame.height);
    glTexCoord2f(1.0f, 0.0f);
    glVertex2f(new_frame.width, 0.0f);
    glEnd();

    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    mlt_events_fire(MLT_CONSUMER_PROPERTIES(&xgl->parent),
                    "consumer-frame-show",
                    mlt_event_data_from_frame(new_frame.mlt_frame_ref));
    ...
    while (vthread.running) {
        ...
        if (!mlt_properties_get_int(MLT_FILTER_PROPERTIES(glsl_manager), "glsl_supported")) {
            mlt_log_error(MLT_CONSUMER_SERVICE(consumer),
                          "OpenGL Shading Language is not supported on this machine.\n");
            xgl->running = 0;
            break;
        }
        ...
        if (next) {
            ...
            if (mlt_properties_get_int(properties, "rendered") == 1) {
                ...
                GLuint *image = 0;
                int error
                    = mlt_frame_get_image(next, (uint8_t **) &image, &vfmt, &width, &height, 0);
                if (!error && image && width > 0 && height > 0 && !new_frame.new) {
                    ...
                    new_frame.aspect_ratio = ((double) width / (double) height)
...
static void initGL(void)
{
    ...
    GLXSWAPINTERVALSGI mglXSwapInterval = (GLXSWAPINTERVALSGI) glXGetProcAddressARB(
...
static void createGLWindow()
{
    ...
    int attrListSgl[]
        = {GLX_RGBA, GLX_RED_SIZE, 8, GLX_GREEN_SIZE, 8, GLX_BLUE_SIZE, 8, GLX_DEPTH_SIZE, 16, None};
    ...
    GLWin.win = XCreateWindow(GLWin.dpy,
                              RootWindow(GLWin.dpy, vi->screen),
                              0,
                              0,
                              width,
                              height,
        ...
        while (XPending(GLWin.dpy) > 0) {
            ...
            case KeyPress:
                ...
                case XK_Escape:
                    ...
                    break;
                default: {
                    mlt_producer producer = mlt_properties_get_data(xgl->properties,
                                                                    "transport_producer",
                                                                    NULL);
                    char keyboard[2] = " ";
                    void (*callback)(mlt_producer, char *)
                        = mlt_properties_get_data(xgl->properties, "transport_callback", NULL);
...
mlt_consumer consumer_xgl_init(mlt_profile profile, mlt_service_type type, const char *id, char *arg)
{
    ...
    if (this != NULL && mlt_consumer_init(&this->parent, this, profile) == 0) {
        ...
        if (glsl_manager) {
            mlt_events_listen(this->properties,
                              &hiddenctx,
                              "consumer-thread-started",
                              (mlt_listener) on_consumer_thread_started);
        } else {
