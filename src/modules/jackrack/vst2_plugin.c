...
static void
vst2_plugin_swap_aux_ports (vst2_plugin_t * plugin, vst2_plugin_t * other)
{
  guint copy;
...
/** connect up the ladspa instance's input buffers to the previous
    ...
    exists. */
void
vst2_plugin_connect_input_ports (vst2_plugin_t * plugin, LADSPA_Data ** inputs)
{
  gint copy;
  ...
  for (copy = 0; copy < plugin->copies; copy++)
    {
      for (channel = 0; channel < plugin->desc->channels; channel++)
        {
	  plugin->holders[copy].effect->setParameter (plugin->holders[copy].effect, plugin->desc->audio_input_port_indicies[channel]-(plugin->holders[copy].effect->numInputs+plugin->holders[copy].effect->numOutputs), *inputs[rack_channel]);
          rack_channel++;
        }
    }
...
void
vst2_plugin_connect_output_ports (vst2_plugin_t * plugin)
{
  gint copy;
  unsigned long channel;
  ...
  for (copy = 0; copy < plugin->copies; copy++)
    {
	  ...
	  plugin->holders[copy].effect->setParameter (plugin->holders[copy].effect, plugin->desc->audio_input_port_indicies[channel]-(plugin->holders[copy].effect->numInputs+plugin->holders[copy].effect->numOutputs), *plugin->audio_output_memory[rack_channel]);
          rack_channel++;
        }
    }
}
...
void
vst2_process_add_plugin (vst2_process_info_t * procinfo, vst2_plugin_t * plugin)
{
...
vst2_plugin_t *
vst2_process_remove_plugin (vst2_process_info_t * procinfo, vst2_plugin_t *plugin)
{
...
void
vst2_process_ablise_plugin (vst2_process_info_t * procinfo, vst2_plugin_t *plugin, gboolean enable)
{
...
/** enable/disable a plugin */
void
vst2_process_ablise_vst2_plugin_wet_dry (vst2_process_info_t * procinfo, vst2_plugin_t *plugin, gboolean enable)
{
...
void
vst2_process_move_plugin (vst2_process_info_t * procinfo, vst2_plugin_t *plugin, gint up)
{
...
vst2_plugin_t *
vst2_process_change_plugin (vst2_process_info_t * procinfo,
	               vst2_plugin_t *plugin, vst2_plugin_t * new_plugin)
{
...
static int
vst2_plugin_open_plugin (vst2_plugin_desc_t * desc,
			 void ** dl_handle_ptr,
...
static int
vst2_plugin_instantiate (const AEffect * effect,
...
static void
vst2_plugin_create_aux_ports (vst2_plugin_t * plugin, guint copy, vst2_context_t * vst2_context)
{
  ...
  for (i = 0; i < desc->aux_channels; i++, aux_channel++)
    {
      ...
      holder->aux_ports[i] =
        jack_port_register (vst2_context->procinfo->jack_client,
                            port_name,
...
static void
vst2_plugin_init_holder (vst2_plugin_t * plugin,
			 guint copy,
			 AEffect *effect,
  ...
  for (i = 0; i < desc->control_port_count; i++)
    {
      ...
      holder->control_memory[i] =
        vst2_plugin_desc_get_default_control_value (desc, desc->control_port_indicies[i], vst2_sample_rate);
      holder->effect->setParameter (holder->effect, desc->control_port_indicies[i]-(holder->effect->numInputs+holder->effect->numOutputs), *(holder->control_memory + i));
    }
...
vst2_plugin_t *
vst2_plugin_new (vst2_plugin_desc_t * desc, vst2_context_t * vst2_context)
{
  void * dl_handle;
...
void
vst2_plugin_destroy (vst2_plugin_t * plugin)
{
  unsigned long i, j;
  int err;
