...
jack_nframes_t vst2_sample_rate;
jack_nframes_t vst2_buffer_size;
...
/* #ifdef WITH_JACK
   ...
   {
     vst2_process_info_t * procinfo = data;
...
void vst2_process_control_port_messages (vst2_process_info_t * procinfo) {
...
static int get_jack_buffers (vst2_process_info_t * procinfo, jack_nframes_t frames) {
  unsigned long channel;
...
vst2_plugin_t *
vst2_get_first_enabled_plugin (vst2_process_info_t * procinfo)
{
  vst2_plugin_t * first_enabled;
...
vst2_plugin_t *
vst2_get_last_enabled_plugin (vst2_process_info_t * procinfo)
{
  vst2_plugin_t * last_enabled;
...
void
vst2_connect_chain (vst2_process_info_t * procinfo, jack_nframes_t frames)
{
  vst2_plugin_t * first_enabled, * last_enabled, * plugin;
  gint copy;
  unsigned long channel;
  if (!procinfo->chain) return;

  first_enabled = vst2_get_first_enabled_plugin (procinfo);
  if (!first_enabled) return;

  last_enabled = vst2_get_last_enabled_plugin (procinfo);
      ...
      if (plugin->desc->aux_channels > 0 && plugin->enabled)
        {
                ...
                for (channel = 0; channel < plugin->desc->aux_channels; channel++)
		  {
		    plugin->holders[copy].
		      effect->setParameter (
					    plugin->holders[copy].effect,
					    plugin->desc->audio_aux_port_indicies[channel]
					    -
					    (plugin->holders[copy].effect->numInputs+plugin->holders[copy].effect->numOutputs),
					    *((LADSPA_Data *) jack_port_get_buffer(plugin->holders[copy].aux_ports[channel], frames))
					    );
		  }
            }
          else
#endif
            {
              ...
              for (copy = 0; copy < plugin->copies; copy++)
                for (channel = 0; channel < plugin->desc->aux_channels; channel++)
                  {
		    plugin->holders[copy].effect->
		      setParameter (plugin->holders[copy].effect,
				    plugin->desc->audio_aux_port_indicies[channel]-(plugin->holders[copy].effect->numInputs+plugin->holders[copy].effect->numOutputs),
  ...
  vst2_plugin_connect_output_ports (first_enabled);
  if (first_enabled != last_enabled)
    {
      vst2_plugin_connect_input_ports (last_enabled, last_enabled->prev->audio_output_memory);
      for (plugin = first_enabled->next; plugin; plugin = plugin->next)
        {
          if (plugin->enabled)
            {
              vst2_plugin_connect_input_ports (plugin, plugin->prev->audio_output_memory);
              vst2_plugin_connect_output_ports (plugin);
            }
        }
    }
  ...
  if( plugin->desc->has_input )
    vst2_plugin_connect_input_ports (first_enabled, procinfo->jack_input_buffers);
}

void
vst2_process_chain (vst2_process_info_t * procinfo, jack_nframes_t frames)
{
  vst2_plugin_t * first_enabled;
  vst2_plugin_t * last_enabled = NULL;
  vst2_plugin_t * plugin;
  unsigned long channel;
  unsigned long i;
  ...
  for (plugin = first_enabled;
       ...
       plugin = plugin->next)
    {
      if (plugin->enabled)
	    ...
	    {
	      if (plugin->holders[i].effect->processReplacing != NULL)
		{
		  plugin->holders[i].effect->
		    processReplacing(plugin->holders[i].effect,
				     plugin->audio_input_memory, plugin->audio_output_memory,
				     frames);
		}
	      else
		{
		  plugin->holders[i].effect->process(plugin->holders[i].effect,
						     plugin->audio_input_memory, plugin->audio_output_memory,
						     frames); // deprecated only few plugins support it
		}
	    }
...
int process_vst2 (vst2_process_info_t * procinfo, jack_nframes_t frames,
		  LADSPA_Data ** inputs, LADSPA_Data ** outputs) {
  unsigned long channel;
  
  if (!procinfo)
    {
      mlt_log_error( NULL, "%s: no vst2_process_info from jack!\n", __FUNCTION__);
      return 1;
    }
  ...
  vst2_process_control_port_messages (procinfo);
  
  for (channel = 0; channel < procinfo->channels; channel++)
    {
      if(vst2_get_first_enabled_plugin (procinfo)->desc->has_input)
        {
  ...
  vst2_connect_chain (procinfo, frames);
  
  vst2_process_chain (procinfo, frames);
...
int vst2_process_jack (jack_nframes_t frames, void * data) {
  int err;
  vst2_process_info_t * procinfo;
  
  procinfo = (vst2_process_info_t *) data;
...
/* static int
   vst2_process_info_connect_jack (vst2_process_info_t * procinfo)
   {
     ...
     jack_set_vst2_process_callback (procinfo->jack_client, vst2_process_jack, procinfo);
...
static void
vst2_process_info_connect_port (vst2_process_info_t * procinfo,
                           gshort in,
...
static int
vst2_process_info_set_port_count (vst2_process_info_t * procinfo,
    ...
    for (in = 0; in < 2; in++)
      {
        ...
        *port_ptr =  jack_port_register (procinfo->jack_client,
        ...
        if ((in && connect_inputs) || (!in && connect_outputs))
          vst2_process_info_connect_port (procinfo, in, i, port_name);
...
void
vst2_process_info_set_channels (vst2_process_info_t * procinfo,
	unsigned long channels, gboolean connect_inputs, gboolean connect_outputs)
{
#ifdef WITH_JACK
  vst2_process_info_set_port_count (procinfo, channels, connect_inputs, connect_outputs);
#endif
  procinfo->channels = channels; 
}

vst2_process_info_t *
vst2_process_info_new (const char * client_name, unsigned long rack_channels, 
	gboolean connect_inputs, gboolean connect_outputs)
{
  ...
  procinfo = g_malloc (sizeof (vst2_process_info_t));
  ...
  if ( client_name == NULL )
    {
      vst2_sample_rate = 48000; // should be set externally before calling process_vst2
      vst2_buffer_size = MAX_BUFFER_SIZE;
      procinfo->silent_buffer = g_malloc (sizeof (LADSPA_Data) * vst2_buffer_size );
...
/* #ifdef WITH_JACK
     err = vst2_process_info_connect_jack (procinfo);
     if (err)
       {
   /\*      g_free (procinfo); *\/
         return NULL;
   /\*      abort (); *\/
     ...
     vst2_sample_rate = jack_get_sample_rate (procinfo->jack_client);
     vst2_buffer_size = jack_get_sample_rate (procinfo->jack_client);
     ...
     err = vst2_process_info_set_port_count (procinfo, rack_channels, connect_inputs, connect_outputs);
     if (err)
       return NULL;
   #endif */
...
void
vst2_process_info_destroy (vst2_process_info_t * procinfo) {
#ifdef WITH_JACK
  if (procinfo->jack_client)
    {
...
void vst2_process_quit (vst2_process_info_t * procinfo) {
  procinfo->quit = TRUE;
}
