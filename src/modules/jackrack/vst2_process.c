...
static int get_jack_buffers(vst2_process_info_t *procinfo, jack_nframes_t frames)
{
    ...
    for (channel = 0; channel < procinfo->channels; channel++) {
        procinfo->jack_input_buffers[channel]
            = jack_port_get_buffer(procinfo->jack_input_ports[channel], frames);
        ...
        procinfo->jack_output_buffers[channel]
            = jack_port_get_buffer(procinfo->jack_output_ports[channel], frames);
...
vst2_plugin_t *vst2_get_first_enabled_plugin(vst2_process_info_t *procinfo)
{
...
vst2_plugin_t *vst2_get_last_enabled_plugin(vst2_process_info_t *procinfo)
{
...
void vst2_connect_chain(vst2_process_info_t *procinfo, jack_nframes_t frames)
{
            ...
            if (procinfo->jack_client) {
                for (copy = 0; copy < plugin->copies; copy++)
                    for (channel = 0; channel < plugin->desc->aux_channels; channel++) {
                        plugin->holders[copy].effect->setParameter(
                            ...
                            *((LADSPA_Data *)
                                  jack_port_get_buffer(plugin->holders[copy].aux_ports[channel],
                                                       frames)));
                    }
            } else
#endif
    ...
    if (plugin && plugin->desc->has_input)
        vst2_plugin_connect_input_ports(first_enabled, procinfo->jack_input_buffers);
}

void vst2_process_chain(vst2_process_info_t *procinfo, jack_nframes_t frames)
{
    vst2_plugin_t *first_enabled;
        ...
        if (plugin->enabled) {
            for (i = 0; i < plugin->copies; i++) {
                if (plugin->holders[i].effect->processReplacing != NULL) {
                    plugin->holders[i].effect->processReplacing(plugin->holders[i].effect,
                                                                plugin->audio_input_memory,
                                                                plugin->audio_output_memory,
                                                                frames);
                } else {
                    plugin->holders[i]
                        .effect->process(plugin->holders[i].effect,
                                         plugin->audio_input_memory,
                                         plugin->audio_output_memory,
...
int process_vst2(vst2_process_info_t *procinfo,
...
static void vst2_process_info_connect_port(vst2_process_info_t *procinfo,
                                           gshort in,
...
static int vst2_process_info_set_port_count(vst2_process_info_t *procinfo,
...
void vst2_process_info_set_channels(vst2_process_info_t *procinfo,
...
vst2_process_info_t *vst2_process_info_new(const char *client_name,
...
void vst2_process_info_destroy(vst2_process_info_t *procinfo)
{
#ifdef WITH_JACK
    if (procinfo->jack_client) {
