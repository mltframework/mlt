...
static mlt_frame filter_process(mlt_filter filter, mlt_frame frame)
{
    ...
    mlt_properties_set_data(unique,
                            "points",
                            points,
                            count * sizeof(BPointF),
                            (mlt_destructor) mlt_pool_release,
                            NULL);
    mlt_properties_set_int(unique, "mode", stringValue(modeStr, MODESTR, 3));
    mlt_properties_set_int(unique,
                           "alpha_operation",
                           stringValue(mlt_properties_get(properties, "alpha_operation"),
                                       ALPHAOPERATIONSTR,
                                       5));
    ...
    mlt_properties_set_int(unique,
                           "feather_passes",
                           mlt_properties_get_int(properties, "feather_passes"));
...
*/
mlt_filter filter_rotoscoping_init(mlt_profile profile,
                                   ...
                                   char *arg)
{
    ...
    if (filter) {
        ...
        mlt_events_listen(properties,
                          filter,
                          "property-changed",
                          (mlt_listener) rotoPropertyChanged);
    }
    return filter;
}
