...
mlt_consumer consumer_cbrts_init(mlt_profile profile,
...
static void load_sections(consumer_cbrts self, mlt_properties properties)
{
    ...
    while (n--) {
        ...
        if (strncmp("si.", name, 3) == 0 && strncmp(".file", name + strlen(name) - 5, 5) == 0) {
...
{
    mlt_properties properties = mlt_properties_get_data(MLT_CONSUMER_PROPERTIES(&self->parent),
                                                        "si.properties",
                                                        NULL);
...
static int create_socket(consumer_cbrts self)
{
    ...
    if (mlt_properties_get(properties, "udp.ttl")) {
        ...
        if (result < 0) {
            mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent), "Error setting the multicast TTL.\n");
            close(self->fd);
            freeaddrinfo(self->addr);
            self->addr = NULL;
            return result;
    ...
    if (mlt_properties_get(properties, "udp.interface")) {
        ...
        if (iface) {
            if (addr->ai_addr->sa_family == AF_INET) {
                ...
                result
                    = setsockopt(self->fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &iface, sizeof(iface));
            }
            if (result < 0) {
                mlt_log_error(MLT_CONSUMER_SERVICE(&self->parent),
...
static void *output_thread(void *arg)
{
    ...
    while (self->thread_running) {
        ...
        while (self->thread_running && i-- && result >= 0) {
            ...
            size_t size = self->rtp_ssrc ? RTP_BYTES + self->udp_packet_size
...
static int output_cbr(consumer_cbrts self, uint64_t input_rate, uint64_t output_rate, uint64_t *pcr)
{
    ...
    mlt_log_debug(NULL,
                  "%s: n %i output_counter %" PRIu64 " input_rate %" PRIu64 "\n",
                  __FUNCTION__,
                  n,
                  self->output_counter,
                  input_rate);
    while (self->thread_running && n-- && result >= 0) {
        ...
        // Check for overflow
        if (input_rate > output_rate && !HASPCR(packet) && pid != SDT_PID && pid != PAT_PID
            && pid != self->pmt_pid) {
            if (!warned) {
                mlt_log_warning(MLT_CONSUMER_SERVICE(&self->parent),
                                "muxrate too low %" PRIu64 " > %" PRIu64 "\n",
                                input_rate,
                                output_rate);
                warned = 1;
            }
        ...
        if (pcr_pid && ms_since_pcr >= PCR_PERIOD_MS && ms_to_end > PCR_PERIOD_MS / 2.0) {
            ...
            if (ms_since_pcr > 40)
                mlt_log_warning(NULL,
                                "exceeded PCR interval %.2f ms queued %.2f ms\n",
                                ms_since_pcr,
                                ms_to_end);
        ...
        while (self->thread_running
               && input_counter + (TSP_BYTES * 8 * input_rate) <= self->output_counter) {
            // See if we need to output a dummy packet with PCR
            ms_since_pcr = (float) (packets_since_pcr + 1) * 8 * TSP_BYTES * 1000 / output_rate;
            ms_to_end = (float) n * 8 * TSP_BYTES * 1000 / input_rate;
...
static inline int filter_packet(consumer_cbrts self, uint8_t *packet)
{
    ...
    int result = (pid == NULL_PID) || (pid == PAT_PID && self->is_si_pat)
...
static void on_data_received(mlt_properties properties,
                             mlt_consumer consumer,
                             mlt_event_data event_data)
{
    buffer_t *buffer = mlt_event_data_to_object(event_data);
...
static int consumer_start(mlt_consumer parent)
{
    ...
    if (!self->running) {
        ...
        mlt_properties_set_data(avformat,
                                "app_lock",
        ...
        if (mlt_properties_get(properties, "udp.address")) {
            if (create_socket(self) >= 0) {
                ...
                self->udp_packet_size = mlt_properties_get_int(properties, "udp.nb_tsp")
                                        * TSP_BYTES;
                ...
                self->femto_per_packet = 1000000000000000ULL * self->udp_packet_size * 8
                                             / self->muxrate
                                         - self->nsec_per_packet * 1000000;
#endif
