...
static int sliced_pix_fmt_conv_proc( int id, int idx, int jobs, void* cookie )
{
	...
	mlt_log_debug( NULL, "%s:%d: [id=%d, idx=%d, jobs=%d], interlaced=%d, field=%d, slices=%d, mul=%d, h=%d, slice_h=%d, slice_y=%d ctx->desc=[log2_chroma_h=%d\n",
	...
	mlt_image_format *format, int width, int height, uint8_t **alpha )
{
	...
	else
	{
		...
		if ( frame->interlaced_frame )
		{
			...
			sliced_pix_fmt_conv_proc( 0, 0, 1, &ctx );
	}
	return result;
}
