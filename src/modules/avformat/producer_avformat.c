...
static int sliced_pix_fmt_conv_proc( int id, int idx, int jobs, void* cookie )
{
...
#if 1 /* force progressive/interlaced source */
	...
	mlt_log_error( NULL, "%s:%d: [id=%d, idx=%d, jobs=%d], interlaced=%d, field=%d, slices=%d, mul=%d, h=%d, slice_h=%d, slice_y=%d ctx->desc=[log2_chroma_h=%d, \"%s\"\n",
		__FUNCTION__, __LINE__, id, idx, jobs, interlaced, field, slices, mul, h, slice_h, slice_y, ctx->desc->log2_chroma_h, ctx->desc->alias );
	...
	mlt_image_format *format, int width, int height, uint8_t **alpha )
{
	...
	else
	{
		...
		sliced_pix_fmt_conv_proc( 0, 0, 6, &ctx );
		sliced_pix_fmt_conv_proc( 0, 1, 6, &ctx );
		sliced_pix_fmt_conv_proc( 0, 2, 6, &ctx );
		sliced_pix_fmt_conv_proc( 0, 3, 6, &ctx );
		sliced_pix_fmt_conv_proc( 0, 4, 6, &ctx );
		sliced_pix_fmt_conv_proc( 0, 5, 6, &ctx );
	}
	return result;
}
