...
static int copy_lines_sliced_proc(int id, int idx, int jobs, void *cookie)
{
    ...
    {
        for (i = 0; i < H; i++) {
            uint32_t val, *v210 = (uint32_t *) (ctx->in_buffer + (Y + i) * ctx->in_stride);
            uint16_t *y = (uint16_t *) (ctx->out_buffers[0] + (Y + i) * ctx->out_strides[0]),
...
static void copy_lines(BMDPixelFormat in_fmt,
                       unsigned char *in_buffer,
                       int in_stride,
                       mlt_image_format out_fmt,
                       unsigned char *out_buffers[4],
                       int out_strides[4],
                       int w,
                       int h)
{
    struct copy_lines_sliced_desc ctx
        = {in_fmt, out_fmt, in_buffer, out_buffers, in_stride, out_strides, w, h};
...
class DeckLinkProducer : public IDeckLinkInputCallback
{
private:
        ...
        if (m_decklinkInput->GetDisplayModeIterator(&iter) == S_OK) {
            while (!result && iter->Next(&mode) == S_OK) {
                ...
                if (width == profile->width && p == profile->progressive
                    && (height + vancLines == profile->height
                        || (height == 486 && profile->height == 480 + vancLines))
...
public:
    ...
    mlt_producer getProducer() const { return m_producer; }
        ...
        try {
            ...
            if (displayMode == (BMDDisplayMode) bmdModeUnknown) {
                mlt_log_info(getProducer(),
                             ...
                             profile->progressive ? "progressive" : "interlace");
            ...
            m_pixel_format
                = (10 == mlt_properties_get_int(MLT_PRODUCER_PROPERTIES(getProducer()), "bitdepth"))
                      ? bmdFormat10BitYUV
                      : bmdFormat8BitYUV;
            BMDVideoInputFlags flags = doesDetectFormat ? bmdVideoInputEnableFormatDetection
            ...
            int channels = mlt_properties_get_int(MLT_PRODUCER_PROPERTIES(getProducer()),
                                                  "channels");
    ...
    mlt_frame getFrame()
    {
        ...
        if (frame) {
            ...
            mlt_properties_set_int(properties,
                                   "format",
                                   (m_pixel_format == bmdFormat8BitYUV) ? mlt_image_yuv422
                                                                        : mlt_image_yuv422p16);
            ...
            mlt_properties_set_int(properties,
                                   "audio_channels",
                                   mlt_properties_get_int(MLT_PRODUCER_PROPERTIES(getProducer()),
                                                          "channels"));
        } else
                                                             ...
                                                             IDeckLinkAudioInputPacket *audio)
    {
        ...
        if (video) {
            ...
            if (!(video->GetFlags() & bmdFrameHasNoInputSource)) {
                ...
                mlt_image_format fmt = (m_pixel_format == bmdFormat8BitYUV) ? mlt_image_yuv422
                                                                            : mlt_image_yuv422p16;
                int size = mlt_image_format_size(fmt,
                                                 video->GetWidth(),
                                                 video->GetHeight() + m_vancLines,
                                                 NULL);
                void *image = mlt_pool_alloc(size);

                mlt_image_format_planes(fmt,
                                        video->GetWidth(),
                                        video->GetHeight() + m_vancLines,
                                        image,
                                        image_buffers,
                ...
                if (m_vancLines > 0) {
                    ...
                    if (video->GetAncillaryData(&vanc) == S_OK && vanc) {
                        for (int i = 1; i < m_vancLines + 1; i++) {
                            unsigned char *out[4] = {image_buffers[0] + (i - 1) * image_strides[0],
        ...
        // Put frame in queue
        if (frame) {
            mlt_properties_set_int64(MLT_FRAME_PROPERTIES(frame),
                                     "arrived",
            ...
            if (mlt_deque_count(m_queue) < queueMax) {
                ...
                mlt_frame_close(frame);
                mlt_properties_set_int(MLT_PRODUCER_PROPERTIES(getProducer()),
                                       "dropped",
                                       ++m_dropped);
                            ...
                            BMDDetectedVideoInputFormatFlags flags)
    {
        ...
        if (events & bmdVideoInputFieldDominanceChanged) {
            ...
            m_topFieldFirst = mode->GetFieldDominance() == bmdUpperFieldFirst;
            mlt_log_verbose(getProducer(),
                            "field dominance changed prog %d tff %d\n",
                            profile->progressive,
                            m_topFieldFirst);
        }
...
mlt_producer producer_decklink_init(mlt_profile profile,
