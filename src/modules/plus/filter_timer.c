...
static void get_timer_str(mlt_filter filter, mlt_frame frame, char *text)
{
    ...
    double value = time_to_seconds(
        mlt_properties_frames_to_time(properties, current_frame, mlt_time_clock));
    ...
    if (timer_duration <= 0.0) {
        ...
        double filter_duration = time_to_seconds(
            mlt_properties_frames_to_time(properties, filter_length, mlt_time_clock));
