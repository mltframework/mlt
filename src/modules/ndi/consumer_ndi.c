            ...
            if (rendered && !mlt_frame_get_image(frm, &image, &format, &width, &height, 0)) {
                ...
                const NDIlib_video_frame_t ndi_video_frame
                    = {// Resolution
                       ...
                       // Use YCbCr video
                       m_isKeyer ? NDIlib_FourCC_type_BGRA : NDIlib_FourCC_type_UYVY,
                ...
                int frequency = 48000;
                int m_channels = 2;
                int samples = mlt_audio_calculate_frame_samples(mlt_profile_fps(profile),
                                                                frequency,
                                                                self->count);
                int16_t *pcm = 0;

                if (!mlt_frame_get_audio(frm,
                                         (void **) &pcm,
                                         &aformat,
                                         &frequency,
                                         &m_channels,
