...
void mlt_events_init(mlt_properties self)
{
    ...
    if (!events && self) {
        ...
        if (events) {
            ...
            events->owner = self;
            mlt_properties_set_data(self,
                                    "_events",
                                    events,
                                    0,
                                    (mlt_destructor) mlt_events_close,
                                    NULL);
        }
    }
}
        ...
        if (listeners != NULL) {
            ...
            if (event == NULL) {
                ...
                if (event != NULL) {
                    ...
                    sprintf(temp,
                            "%d",
                            first_null == -1 ? mlt_properties_count(listeners) : first_null);
                    event->parent = events;
                    event->ref_count = 0;
                    event->block_count = 0;
                    event->listener = listener;
                    event->listener_data = listener_data;
                    mlt_properties_set_data(listeners,
                                            temp,
                                            event,
                                            0,
                                            (mlt_destructor) mlt_event_close,
                                            NULL);
